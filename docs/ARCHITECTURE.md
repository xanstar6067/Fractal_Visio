...
| 6 | 🔶 **Частично.** `UiRouter` (модуль), `UiScreen`, матовое стекло (`BackdropBlur`, `GlassPanel`, `UiSprites`, `UiTheme`), блокировка жестов через `PointerOverUi`. Панель настроек — прокручиваемая колонка `SettingsSection` с секциями FRACTAL / PALETTE / COLOURING / RESOLUTION / INTERFACE SIZE. Масштаб UI: `ScreenScale.Density` (максимум из заявленного dpi и того, что следует из разрешения) × `UserScale` (0.6…2.5). Осталось: авто-генерация контролов по `FractalParameterDescriptor`, экран закладок, редактор палитр | низкий |
...
**Грубый проход не заменяет более чёткий кадр.** Презентер передаёт в запрос `minimumPublishStep`:
пока то, что уже на экране, покрывает текущий вид (`FramePlacement.Covers`), проходы грубее шага 4
считаются, но не публикуются — увеличенный чёткий кадр выглядит лучше свежего блочного. Последний
проход прогона публикуется всегда, иначе урезанный интерактивный рендер не дал бы ничего.
