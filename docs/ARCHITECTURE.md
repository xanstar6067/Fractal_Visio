...
Два дополнения к тому же этапу:

- Непокрытые репроекцией пиксели помечать как «неизвестно» и отдавать им приоритет в первом
  же проходе, вместо `clamp`. Тогда даже за пределами полей полоса живёт доли секунды.
- Билинейная выборка вместо `Math.Round` в `ReprojectFrame` — убирает дрожание при
  медленной панораме. Полосы этим не лечатся, но качество репроекции заметно ровнее.
...
| 2 | **Оверскан** (см. 4.7). Сделано: поля в `Viewport`, `ResolveCpuViewport`, `ViewNavigator.ForViewport`, `uvRect` в контроллере. Осталось: маска непокрытых пикселей с приоритетом в первом проходе, ограничение полей коэффициентом прохода, билинейная репроекция | средний — трогает `ReprojectFrame` и пассы, проверять на глубоком зуме |
...
| 8 | **Второй фрактал** (Burning Ship) как проверка шаблона | низкий |
